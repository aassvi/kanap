...
*/
function checkQuantity(product,product_panier){
  ...
  const noze = console.log(" function ChangeQuantity formQuantity =  "+formQuantity[0].value);
  const noae = console.log(" function ChangeQuantity formQuantity =  "+formQuantity[1].value);
  
const li = console.log("test  product id--------------------" + product._id);
const lis = console.log("test pannier quantite--------------------" + product_panier.quantiteProduitPanier);
const lisp = console.log("test pannier color--------------------" + product_panier.colorProduitPanier);

  // faire la boucle indexer element.addEvent
  for (var c = 0; c < panier.length; c++) {
      const limsp = console.log("test pannier boucle function change --------------------" );
      formQuantity[c].addEventListener('change',(event) =>{
        ...
        ancienneQuantity = product_panier.quantiteProduitPanier ;
        const ancQT = console.log(" function ChangeQuantity ancienneQuantity =  " + ancienneQuantity);
        
        const lit = console.log("test  product id--------------------" + product._id);
        const liis = console.log("test pannier id--------------------" + product_panier.idProduitPanier);
        const lisyp = console.log("test pannier color--------------------" + product_panier.colorProduitPanier);
...
function changeQuantity(product,nouvelleQuantity){
  let panier = getPanierLS();
  let foundProduct = panier.find(p =>p.idProduitPanier == product._id );
  const fg= console.log(" f change------- foundproduct   "+ foundProduct.quantiteProduitPanier );
  if (foundProduct != undefined  &&  foundProduct.quantiteProduitPanier!=nouvelleQuantity){
